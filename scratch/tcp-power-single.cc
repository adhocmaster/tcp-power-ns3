...
NS_LOG_COMPONENT_DEFINE ("TcpVenoRunner");
                      ...
                      Callback <void, uint32_t, uint32_t> CwndTrace)
{
  ...
  Simulator::Schedule (Seconds (10.0) + Seconds (0.00001), &TraceCwnd, nodeId, cwndWindow, CwndTrace);
...
int main (int argc, char *argv[])
{
  ...
  LogComponentEnable("TcpVenoRunner", LOG_LEVEL_INFO);
  //LogComponentEnable("TcpVeno", LOG_LEVEL_INFO);
  LogComponentEnable("TcpPW", LOG_LEVEL_INFO);
  LogComponentEnable("TcpCongestionOps", LOG_LEVEL_INFO);

  uint32_t initialWindow = 10;

  uint32_t stream = 1;
  std::string socketFactory = "ns3::TcpSocketFactory";
  //std::string tcpTypeId = "ns3::TcpVeno";
  std::string tcpTypeId = "ns3::TcpPW";
  ...
  CommandLine cmd;
  cmd.AddValue ("initialWindow", "Initial window size in # packets", initialWindow);
  ...
  Config::SetDefault ("ns3::TcpSocket::InitialCwnd", UintegerValue (initialWindow));
